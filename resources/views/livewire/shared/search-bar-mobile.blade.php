{{-- Mobile Version - Only the input field container --}}
<form wire:submit.prevent="search" class="relative w-5/6 mx-auto mb-4" x-data="{ focused: false }" @click.outside="$wire.hideResults()">
    ...
    <input type="text" 
        ...
        class="h-12 pl-12 placeholder:font-robotoCondensed placeholder:text-color-2 placeholder:font-light w-full p-2 bg-white text-color-2 border border-gray-300 font-robotoCondensed focus:outline-none transition-all duration-200 {{ $showResults ? 'rounded-t-full border-b-0' : 'rounded-full' }}" 
    ...
    @if($showResults)
        <div class="absolute top-full left-0 w-full bg-white border border-gray-300 border-t-0 rounded-b-[2.5rem] shadow-lg z-[60] max-h-80 overflow-y-auto">
                           ...
                           class="flex items-center p-2 hover:bg-gray-50 rounded-lg transition-colors duration-150">
                            @if($product['image'])
                                <img src="{{ $product['image'] }}" 
                                     alt="{{ $product['name'] }}" 
                                     class="w-10 h-10 object-cover rounded-lg mr-3 bg-gray-100">
                            @else
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        @if($product['discounted_price'])
                                            <span class="text-xs font-bold text-red-600">€{{ number_format($product['discounted_price'], 2) }}</span>
                                            <span class="text-xs text-gray-400 line-through ml-1">€{{ number_format($product['price'], 2) }}</span>
                                        @else
                                            <span class="text-xs font-bold text-gray-900">€{{ number_format($product['price'], 2) }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
