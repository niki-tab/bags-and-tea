<?php

return [
    'hero_title' => 'Bolsos de Lujo Segunda Mano | Comprar Bolsos de Marca',
    'hero_subtitle' => 'Sin intermediarios',
    'shop_now' => 'Compra Ahora y obten un 10% de descuento',
    'about_title' => 'Sobre Nosotros',
    'about_description_1' => 'Nos comprometemos a a traer los mejores bolsos',
    'about_description_2' => '¡Disfruta de bolsos de marca vintage',
    'contact_us' => 'Contáctanos',
    'featured_products' => 'Mariscos y Pescados Destacados',
    'view_product' => 'Ver Producto',
    'label-product-origin' => 'Origen:',
    'label-product-fresh-stock' => 'Stock:',
    'label-product-price-from' => 'Desde:',
    'label-product-stock-available' => 'Hay existencias',
    'label-product-no-stock-available' => 'No hay existencias',

    'title-sell-your-bag' => 'Vende tu bolso de lujo',   
    'description-sell-your-bag' => 'Bienvenida a B&T, el lugar perfecto par vender, comprar o alquilar artículos de lujo exclusivos, desde un icono atemporal hasta el último it bag.',    
    'button-sell-your-bag' => 'Ir al formulario',

    'title-avantage-buying-bags-and-tea' => 'VENTAJAS DE COMPRAR TU BOLSO DE SEGUNDA MANO DE MARCA EN BAGS AND TEA',  
    'description-avantage-buying-bags-and-tea' => 'En Bags & Tea, comprar un bolso de lujo es mucho más que adquirir un accesorio: es invertir en elegancia, autenticidad y sostenibilidad. Cada pieza ha sido cuidadosamente seleccionada por su calidad, rareza y diseño icónico, garantizando que solo lo excepcional llegue a tus manos. Apostamos por la moda consciente, dando una segunda vida a tesoros que merecen ser preservados. Nuestro proceso de verificación experta te ofrece la tranquilidad de una compra segura y respaldada. Con atención personalizada y una selección exclusiva, en Bags & Tea descubrirás el lujo más atemporal con el cuidado y la confianza que mereces.',  

    'brands_title' => 'NUESTROS BOLSOS DE LUJO DE SEGUNDA MANO',

    'title-featured-products' => 'Nuestros bolsos de segunda mano de lujo más recientes',
    'button-view-all-products' => 'Ver todos',

    'headline-1' => 'Bolsos de Lujo Segunda Mano',
    'headline-2' => 'Encuentra tus bolsos de marca de segunda mano deseados',

    'page-seo-title' => 'Bolsos de Lujo Segunda Mano | Comprar Bolsos de Marca',
    'page-seo-description' => 'Compra y venta de bolsos de lujo de segunda mano con confianza. Selección exclusiva, marcas icónicas y autenticidad garantizada en cada pieza.',

    'faq_title' => 'Preguntas frecuentes sobre la venta de bolsos de lujo en Bags and Tea',
    'faq_1_question' => '¿Es seguro comprar bolsos de lujo de segunda mano por internet?',
    'faq_1_answer' => 'Sí. En B&T nos tomamos la seguridad muy en serio. Cada pieza que ofrecemos ha sido cuidadosamente inspeccionada por expertos para garantizar su autenticidad y estado. Comprar a través de nuestra plataforma es una experiencia segura, transparente y respaldada por un equipo con años de experiencia en la venta de bolsos de lujo de segunda mano.',
    
    'faq_2_question' => '¿Cómo puedo saber si un bolso de marca de segunda mano es auténtico?',
    'faq_2_answer' => 'Solo trabajamos con piezas 100% auténticas. Cada bolso pasa por un riguroso proceso de verificación que combina la experiencia humana con tecnología avanzada. En B&T, la autenticidad no se presupone: se garantiza.',    

    'faq_3_question' => '¿Qué tipo de garantía ofrecéis en la compra de bolsos de lujo de segunda mano?',
    'faq_3_answer' => 'Ofrecemos garantía de autenticidad y una política de devolución flexible, porque sabemos que comprar un bolso de lujo de segunda mano es también una decisión emocional. Queremos que te sientas plenamente segura con tu elección.',

    'faq_4_question' => '¿Los bolsos de segunda mano están en buen estado o tienen desperfectos?',
    'faq_4_answer' => 'Solo seleccionamos bolsos de segunda mano que se encuentren en excelente estado o que conserven su belleza original a pesar del paso del tiempo. Cada artículo incluye una descripción detallada de su condición y fotografías reales para que puedas apreciar cada detalle con claridad.',

    'faq_5_question' => '¿Puedo devolver un bolso si no me convence al recibirlo?',
    'faq_5_answer' => 'Sí, dispones de un plazo para solicitar la devolución si el bolso no cumple tus expectativas. Nuestra prioridad es tu satisfacción y confianza al comprar bolsos de lujo de segunda mano con nosotros.',

    'faq_6_question' => '¿Qué marcas de bolsos de lujo de segunda mano tenéis disponibles?',
    'faq_6_answer' => 'Vendemos bolsos de Chanel, Hermès, Louis Vuitton, Dior, Saint Laurent, Fendi, Loewe, Prada, Gucci, Balenciaga, Céline, Bottega Veneta, y Polène. Estas son las marcas en las que somos expertos. Siempre estamos evolucionando, y esperamos expandir esta lista en el futuro.',
    
    'faq_7_question' => ' ¿Cómo funcionan los envíos y cuánto tardan en llegar los bolsos de segunda mano?',
    'faq_7_answer' => 'Ofrecemos envíos rápidos y asegurados a toda España y otros destinos europeos. En general, tu bolso llegará en un plazo de entre 2 y 5 días laborables, cuidadosamente protegido y listo para ser disfrutado.',
    
    'faq_8_question' => '¿Ofrecéis algún tipo de financiación o pago fraccionado para comprar bolsos de lujo?',
    'faq_8_answer' => 'En B&T no ofrecemos financiación ni pago fraccionado. Cada bolso de lujo de segunda mano está disponible para compra directa, con total transparencia en el precio y sin costes ocultos. Apostamos por una experiencia de compra clara, honesta y alineada con los valores del consumo responsable.',

    'faq_9_question' => '¿Puedo reservar un bolso de lujo de segunda mano mientras decido si comprarlo?',
    'faq_9_answer' => 'Dada la exclusividad de muchas de nuestras piezas, no ofrecemos servicio de reserva. Cada bolso de lujo de segunda mano en nuestra web está disponible hasta que alguien lo adquiere, por lo que recomendamos actuar con rapidez si has encontrado esa pieza especial que llevabas tiempo buscando.',

    'faq_10_question' => '¿Dónde puedo ver vuestra selección completa de bolsos de lujo de segunda mano?',
    'faq_10_answer' => 'Puedes descubrir nuestra colección completa en la web de B&T, donde encontrarás piezas únicas seleccionadas con mimo y actualizadas constantemente. Cada bolso tiene su historia y te está esperando.',
    
];
